...
resource "aws_instance" "DEV-EC2-BAST" {
  count = "1"
  ami           = var.bastion_ami_dev
  ...
  monitoring = "true"
tags = {
    Name = "DEV-EC2-BASTION${count.index + 1}"
  }
}
...
#resource "aws_instance" "DEV-EC2-APP" {
#  count = "1"
#  ami           = var.application_ami_dev
...
##        delete_on_termination = "true"
##      }
#  vpc_security_group_ids = ["${aws_security_group.DEV-EC2-APP-SG.id}"]
#  subnet_id = aws_subnet.dev-public[count.index].id
#  key_name = var.key_pair
#  associate_public_ip_address = true
#  monitoring = "true"
#tags = {
#    Name = "DEV-EC2-APPLICATION${count.index + 1}"
#  }
#}

# EC2 Group resources: This will create 1 EC2 instance in a private subnet for the mapping server

#resource "aws_instance" "DEV-EC2-MAP" {
#  count = "1"
#  ami           = var.mapping_ami_dev
#  instance_type = "t2.small"
#   root_block_device {
#        volume_type           = "gp2"
#        volume_size           = "100"
#        delete_on_termination = "true"
#      }
#  vpc_security_group_ids = ["${aws_security_group.DEV-EC2-MAP-SG.id}"]
#  subnet_id = aws_subnet.dev-public[count.index].id
#  key_name = var.key_pair
#  associate_public_ip_address = true
#  monitoring = "true"
#tags = {
#    Name = "DEV-EC2-MAPPING${count.index + 1}"
#  }
#}

# Define cloudwatch role for EC2 instances

resource "aws_iam_role" "dev-ec2-cloudwatch" {
  name = "dev-ec2-cloudwatch"
  ...
  tags = {
    tag-key = "dev-ec2-cloudwatch"
  }
}
resource "aws_iam_role_policy_attachment" "dev-ec2-cloudwatch" {
  role       = "${aws_iam_role.dev-ec2-cloudwatch.name}"
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}
resource "aws_iam_instance_profile" "dev-ec2-cloudwatch" {
  name  = "dev-ec2-cloudwatch"
  role = "${aws_iam_role.dev-ec2-cloudwatch.name}"
}
